...
try {
  ...
  if (process.argv.includes('--dump-data')) {
    const pick = ['AREAS','DEVELOPERS','PROJECTS','UNITS','RELEASES','PROJECT_GROUPS','RESEARCH',
                  'UNIT_EXTRA','PROJECT_COVERS','PROJECT_GALLERY','PROJECT_BROCHURE','PROJECT_FEATURES','PROJECT_PLANS','UNIT_IMAGES','UNIT_GALLERY','UNIT_MASTERPLANS',
  ...
  ck('listings: Caesar + June (SODIC North Coast) added', api.projBySlug('caesar-north-coast').price===39217000 && api.projBySlug('june-north-coast').price===28973000 && api.projBySlug('caesar-north-coast').dev==='sodic' && api.projBySlug('june-north-coast').dev==='sodic');
  ...
  // ---- UI refinements ----
  api.setFilter(api.defaultFilter());
  ck('count: SODIC shows 87 units dynamically (facet from data)', api.devFacets().sodic===87, 'sodic='+api.devFacets().sodic);
  api.setFilter(api.defaultFilter());
  ...
  /* This asserted that a bare filename gains /project-media/plans/ and a rooted
     path is left alone. Every project has since moved to its own directory, so
     no unit names a bare filename any more and that shared directory is gone —
     planUrl keeps the rule for whatever needs it next, but there is no longer
     data to assert it through. What matters now is the rooted case: every plan
     a unit shows resolves under its own project's media and is a real file. */
  ck('plans: every master and floor plan resolves to a file that exists', (function(){
    var fsx=require('fs'), pathx=require('path'), bad=[];
    api.UNITS.forEach(function(u){
      api.unitMasterplans(u).concat(api.unitFloorplans(u)).forEach(function(s){
        if(s.indexOf('/project-media/') !== 0 ||
           !fsx.existsSync(pathx.join(__dirname, '..', s.replace(/^\//,'')))) bad.push(u.id+':'+s);
      });
    });
    var ofp=api.unitFloorplans(api.unitById('OG-03'));
    return bad.length===0 && ofp.length===2 &&
           ofp.every(function(s){ return s.indexOf('/project-media/ogami/units/')===0; });
  })(), 'ok');
...
/* ---- security posture, asserted against the built file -------------------
  ...
  ck('sodic: every from-price is its project\'s own cheapest unit', (function(){
    ...
    return ps.length === 8 && us.length === 87 && stale.length === 0 &&
  ...
  ck('msquared: terms are claimed only where a price list exists', (function(){
    ...
    if(ps.some(function(p){ return /ashgar/.test(p.slug); })) return false;
  ...
  /* A project's photography strip was invisible to every check here and never
     reached the database at all — PROJECT_GALLERY was neither exported nor
     migrated, so 147 frames across 13 projects existed only in the bundle. It
     is carried now, and this asserts the two things that would go wrong: a
     frame that is not a real file, and a gallery whose project is gone. */
  ck('site: every project gallery frame is a real file on a live project', (function(){
    var fsx=require('fs'), pathx=require('path'), bad=[];
    var live={}; api.PROJECTS.forEach(function(p){ live[p.slug]=1; });
    Object.keys(api.PROJECT_GALLERY).forEach(function(slug){
      if(!live[slug]) { bad.push(slug+':no such project'); return; }
      (api.PROJECT_GALLERY[slug]||[]).forEach(function(src){
        if(typeof src !== 'string' || src.indexOf('/project-media/') !== 0 ||
           !fsx.existsSync(pathx.join(__dirname,'..',src.replace(/^\//,'')))) bad.push(slug+':'+src);
      });
    });
    return bad.length === 0;
  })(), 'ok');
  /* The same gap PROJECT_GALLERY had: the pictures a developer page and a
     project's Digital Brochure are built from were in no check here and in
     neither database tool, so 133 feature images and 116 brochure pages lived
     only in the bundle. Both are carried now; this asserts each is a real file
     on an owner that still exists. */
  ck('site: every feature image and brochure page is a real file on a live owner', (function(){
    var fsx=require('fs'), pathx=require('path'), bad=[];
    var devs={}; api.DEVELOPERS.forEach(function(d){ devs[d.key]=1; });
    var projs={}; api.PROJECTS.forEach(function(p){ projs[p.slug]=1; });
    function file(src){
      return typeof src === 'string' && src.indexOf('/project-media/') === 0 &&
             fsx.existsSync(pathx.join(__dirname,'..',src.replace(/^\//,'')));
    }
    Object.keys(api.DEV_FEATURES).forEach(function(k){
      if(!devs[k]) { bad.push(k+':no such developer'); return; }
      var f = api.DEV_FEATURES[k];
      if(f.masterplan && !file(f.masterplan.src)) bad.push(k+':'+f.masterplan.src);
      (f.cards||[]).forEach(function(c){
        (c.imgs||[]).forEach(function(s){ if(!file(s)) bad.push(k+':'+s); });
      });
    });
    Object.keys(api.PROJECT_BROCHURE).forEach(function(slug){
      if(!projs[slug]) { bad.push(slug+':no such project'); return; }
      (api.PROJECT_BROCHURE[slug].pages||[]).forEach(function(s){ if(!file(s)) bad.push(slug+':'+s); });
    });
    Object.keys(api.PROJECT_FEATURES).forEach(function(slug){
      if(!projs[slug]) { bad.push(slug+':no such project'); return; }
      var f = api.PROJECT_FEATURES[slug];
      if(f.masterplan && !file(f.masterplan.src)) bad.push(slug+':'+f.masterplan.src);
      (f.cards||[]).forEach(function(c){
        (c.imgs||[]).forEach(function(s){ if(!file(s)) bad.push(slug+':'+s); });
      });
    });
    return bad.length === 0;
  })(), 'ok');
  /* A town released in phases shows them inside its own page, and the
     developer page shows the parent alone — otherwise Makadi Heights and the
     two phases it is sold through competed as three sibling cards, which is
     not how the client sheet files them. Each phase stays a full project with
     its own page and units; only where its card appears has changed. */
  ck('phases: the parent lists them, the developer page shows the parent alone', (function(){
    var kids = api.projectPhases('makadi-heights').map(function(p){ return p.slug; });
    if(kids.join(',') !== 'ledge-valley,siyal') return false;
    // each is still a real project of the same developer, with its own units
    var ok = kids.every(function(s){
      var p = api.projBySlug(s);
      return p && p.dev === 'orascom' && api.phaseOf(s) === 'makadi-heights' &&
             api.UNITS.some(function(u){ return u.project === s; });
    });
    // the parent's page names them; the developer page does not repeat them
    var pn = txt(api.V.project('makadi-heights').node);
    var dn = api.V.developer('orascom').node;
    var cards = qsa(dn,'a').map(function(a){ return (a.getAttribute&&a.getAttribute('href'))||''; })
                  .filter(function(h){ return /\/projects\//.test(h); });
    var repeated = kids.filter(function(s){
      return cards.some(function(h){ return h.indexOf('/projects/'+s+'/') > -1; });
    });
  ...
  ck('lmd: the profile is on the page — 18 cards, 55 frames, both languages', (function(){
    ...
    return (bad.length === 0 && frames.length === 55 && under.length === 0) ||
           ('bad='+bad.length+' frames='+frames.length+' stray='+under.length);
  })(), true);
  /* The page had a hole where every other developer shows photographs, and the
     gallery fills it. There is deliberately no plan band: a master plan and a
     location map belong to a project, and a company with twenty-six of them in
     four countries has neither of its own — the profile's drawings sit in the
     card of the project each was drawn for, as with Orascom. */
  ck('lmd: a gallery, and no plan band claiming a company master plan', (function(){
    var fsx=require('fs'), pathx=require('path');
    var g = api.DEV_GALLERY.lmd || [], f = api.DEV_FEATURES.lmd || {};
    var missing = g.filter(function(x){
      return !fsx.existsSync(pathx.join(__dirname,'..',String(x).replace(/^\//,''))); });
    var n = api.V.developer('lmd').node;
    var strip = qsa(n,'.dev-gal').length, band = qsa(n,'.dev-mp').length;
    // the drawings are still on the page, inside the cards that own them
    var inCards = [].concat.apply([], (f.cards||[]).map(function(c){ return c.imgs; }))
      .filter(function(x){ return /-(plan|masterplan|location)\.webp$/.test(x); }).length;
    return (g.length === 13 && missing.length === 0 && strip === 1 && band === 0 &&
            !f.masterplan && !f.plans && inCards === 16) ||
           ('gal='+g.length+' missing='+missing.length+' strip='+strip+' band='+band+' inCards='+inCards);
  })(), true);
  ...
  ck('adagio: a coming-soon card that opens El Masria and claims nothing else', (function(){
    var bad = [];
    var a = api.COMING_SOON_LAUNCHES.filter(function(c){ return c.name === 'Adagio'; })[0];
    if(!a) return 'no Adagio teaser';
    // It opens the developer's page, not a project page it does not have.
    if(a.link) bad.push('links to a project: '+a.link);
    if(a.dev !== 'elmasria') bad.push('dev='+a.dev);
    if(api.csHref(a) !== api.buildPath('developer',{slug:'elmasria'}))
      bad.push('href='+api.csHref(a));
    if(api.comingSoonForDev('elmasria') !== a) bad.push('not found by developer');
    // The title is the DEVELOPERS record's own name, so EN and AR cannot drift.
    var d = api.devByKey('elmasria');
    if(api.csTitle(a) !== d.name[api.lang]) bad.push('title='+api.csTitle(a));
    if(!d.name.en || !d.name.ar) bad.push('developer name is not bilingual');
    // A teaser is a picture and a badge. No price, area, delivery or payment
    // plan — the poster publishes none and we must not invent any.
    ['price','area','dp','years','delivery','finishing','types'].forEach(function(k){
      if(a[k] !== undefined) bad.push('carries '+k);
    });
  ...
  /* This asserted that the replacements exist, which they always did — it never
     ...
     the route itself. */
  /* A project given a master plan by BOTH its brochure and its units' sheet
     rows rendered two full-width bands, stacked, each holding one button. They
     belong in one row. */
  ck('site: master plan and location share one band, never two', (function(){
    var doubled = api.PROJECTS.filter(function(p){
      var f = api.projFeatures(p.slug);
      return f && f.masterplan && (api.projectPlans(p.slug).mp || []).length;
    });
    if(!doubled.length) return false;                 // the case must still exist
    return doubled.every(function(p){
      var sec = api.projectPlanSection(p);
      if(!sec) return false;
      var rows = sec.querySelectorAll('.dev-mp');
      if(rows.length !== 1) return false;
      var btns = rows[0].querySelectorAll('.ufeat');
      return btns.length === 2;                        // master plan + location
    });
  })());
